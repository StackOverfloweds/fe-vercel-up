...
export default {
    ...
    data() {
        return {
            ...
            apiBaseUrl: "https://posmate.my.id", // Replace with your actual API URL
        };
    },
    methods: {
        ...
        async setupData() {
            try {
                ...
                this.transactions = expenseResponse.data || []; // Store the fetched data (default to an empty array)

                // Calculate the total Pengeluaran Belanja (Expenses)
                this.totalPengeluaran = this.transactions.reduce(
                    (total, transaction) => {
                        // Ensure transaction.price is valid and falls back to 0 if invalid
                        const price = parseFloat(transaction.price);
                        const quantity = parseFloat(transaction.quantity);
                        return total + (isNaN(price) ? 0 : price) *quantity; // Add valid price or 0
                    },
                    0
                );

                // Format the totalPengeluaran as Rupiah
                ...
                // Ensure fallback value for totalPengeluaranFormatted
                this.totalPengeluaranFormatted = new Intl.NumberFormat(
                    "id-ID",
                    {
                        style: "currency",
        ...
        async fetchTransactions() {
            try {
                ...
                this.totalPendapatan = salesTransactions.reduce(
                    (total, transaction) => {
                        // Parse the total_amount and ensure it falls back to 0 if invalid
                ...
                // Format the totalPendapatan as Rupiah
                ...
                // Ensure totalPendapatanFormatted is a fallback if an error occurs
                this.totalPendapatanFormatted = new Intl.NumberFormat("id-ID", {
                    style: "currency",
                    currency: "IDR",
                }).format(0); // Default to 0
